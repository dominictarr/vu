...
//var last = this.children[this.childElementCount - 1]
  ...
  reverse: function () {
    ...
    //which is fastor push(shift()) or unshift(pop())
    //the API looks like a linked list.
    //each child has pointers to prev and next
    //which will be reassigned.
    //so, insertion should be constant either way?
    //as long as the element doesn't iterate over the
    //list to check it's actually a member.
    while(l--)
...
function List (item) {
  ...
  el.listener = function (name, args) {
    console.log('reveived', name, args)
    var l = args.length
    var m = name == 'splice' ? 2 : 0
    while(l-- > m)
      args[l] = item.call(args[l])
